...
export function createCalendarManagementStore(userPubkey) {
  ...
  function getDeletedCalendarId(deletionEvent) {
    ...
    const aTag = deletionEvent.tags.find(
      /** @param {any[]} tag */ (tag) => tag[0] === 'a' && tag[1] && tag[1].startsWith('31924:')
    );
  ...
  function initializeLoader() {
    ...
    deletionSubscription = deletionLoader()
      ...
      .subscribe(
        /** @param {any} deletionEvent */ (deletionEvent) => {
    ...
    subscription = timelineLoader()
      ...
      .subscribe(
        /** @param {Calendar | null} calendar */ (calendar) => {
          if (calendar) {
  ...
  async function addEventToCalendar(calendarId, event) {
    try {
      ...
      if (publishResult.success) {
        ...
        store.calendars = store.calendars.map((c) => (c.id === calendarId ? updatedCalendar : c));
        ...
        console.log(
          '📅 Calendar Management: Successfully added event to calendar:',
          calendar.title
        );
        console.log(
          '📅 Calendar Management: Published to',
  ...
  async function removeEventFromCalendar(calendarId, event) {
    try {
      ...
      const updatedEventReferences = calendar.eventReferences.filter(
        (ref) => ref !== eventReference
      );
      ...
      if (publishResult.success) {
        ...
        console.log(
          '📅 Calendar Management: Successfully removed event from calendar:',
          calendar.title
        );
        console.log(
          '📅 Calendar Management: Published to',
