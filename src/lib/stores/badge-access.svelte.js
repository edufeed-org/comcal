...
export function useBadgeAccess(getCommunityEvent) {
  let accessMap = /** @type {SvelteMap<string, {read: boolean, write: boolean}>} */ (
    new SvelteMap()
  );
  let userBadges = /** @type {SvelteSet<string>} */ (new SvelteSet());
  ...
  $effect(() => {
    ...
    if (!userPubkey || !communityEvent) {
      isLoading = false;
      accessMap = new SvelteMap();
      userBadges = new SvelteSet();
      return;
    }
 ...
 * @returns {Promise<SvelteSet<string>>} Set of badge addresses the user holds
 */
