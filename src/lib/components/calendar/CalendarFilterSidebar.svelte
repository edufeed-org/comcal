      ...
      >
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-base font-semibold text-base-content">
            {m.calendar_filter_sidebar_title()}
          </h2>
          {#if hasActiveFilters}
  ...
  {:else}
    ...
    >
      <div class="relative">
        ...
        >
          <FilterIcon
            class_="h-5 w-5 {hasActiveFilters ? 'text-primary' : 'text-base-content/70'}"
          />
        </button>
        {#if hasActiveFilters}
          <span
            class="absolute -top-1 -right-1 badge h-5 min-w-5 p-0 text-xs badge-xs font-semibold badge-primary"
          >
            {activeFilterCount}
          </span>
        {/if}
...
{#if isDrawerOpen}
  <div class="lg:hidden">
    ...
    <!-- Drawer -->
    <aside
      class="fixed top-0 left-0 z-50 flex h-full w-80 max-w-[85vw] flex-col overflow-hidden bg-base-300 shadow-xl transition-transform"
    >
      ...
      <div class="border-t border-base-300 bg-base-200 p-4">
        <button class="btn btn-block" onclick={closeDrawer}
          >{m.calendar_filter_sidebar_close_button()}</button
        >
      </div>
    </aside>
  </div>
