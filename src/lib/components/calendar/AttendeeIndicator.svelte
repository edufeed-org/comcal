...
{#if totalCount > 0}
	{#if compact}
		...
		<div class="flex items-center gap-3 text-sm">
			<div class="flex items-center gap-1 text-base-content/70">
				...
				<span class="font-medium">{m.attendee_indicator_attendees_label({ count: totalCount })}</span>
			</div>
	...
	{:else}
		...
		<div class="space-y-4">
			<h3 class="text-lg font-semibold text-base-content">{m.attendee_indicator_attendees_label({ count: totalCount })}</h3>
			...
			{#if accepted.length > 0}
				<div class="space-y-3">
					<div class="flex items-center gap-2">
						...
						<h4 class="font-medium text-base-content">{m.attendee_indicator_accepted_label({ count: accepted.length })}</h4>
					</div>
			...
			{#if tentative.length > 0}
				<div class="space-y-3">
					<div class="flex items-center gap-2">
						...
						<h4 class="font-medium text-base-content">{m.attendee_indicator_maybe_label({ count: tentative.length })}</h4>
					</div>
					<div class="space-y-2 ml-6">
			...
			{#if declined.length > 0}
				<div class="space-y-3">
					<div class="flex items-center gap-2">
						...
						<h4 class="font-medium text-base-content">{m.attendee_indicator_declined_label({ count: declined.length })}</h4>
					</div>
					<div class="space-y-2 ml-6">
...
{#if isViewAllOpen}
	<div class="modal modal-open">
		<div class="modal-box max-w-2xl">
			<h3 class="text-lg font-bold mb-4">{m.attendee_indicator_modal_title({ count: totalCount })}</h3>
			
			<!-- Tabbed view for different RSVP statuses -->
			<div role="tablist" class="tabs tabs-boxed mb-4">
				<button role="tab" class="tab tab-active">{m.attendee_indicator_accepted_label({ count: accepted.length })}</button>
				{#if tentative.length > 0}
					<button role="tab" class="tab">{m.attendee_indicator_maybe_label({ count: tentative.length })}</button>
				{/if}
				{#if declined.length > 0}
					<button role="tab" class="tab">{m.attendee_indicator_declined_label({ count: declined.length })}</button>
				{/if}
			</div>
			...
			<div class="modal-action">
				<button class="btn" onclick={() => isViewAllOpen = false}>{m.attendee_indicator_modal_close()}</button>
			</div>
		</div>
