...
<span class="break-words">
  {#each segments as segment, i (i)}
    {#if segment.type === 'link'}
      <!-- eslint-disable svelte/no-navigation-without-resolve -- external: dynamically extracted URL -->
      <a
        href={segment.href}
        target="_blank"
        rel="noopener noreferrer"
        class="inline-flex link items-center gap-1 break-all link-primary hover:link-hover"
      >
        {segment.content}
