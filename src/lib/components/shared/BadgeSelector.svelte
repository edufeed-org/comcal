<script>
  ...
  let badges = $state(
    /** @type {Array<{id: string, pubkey: string, identifier: string, name: string, description: string, image: string, thumb: string, address: string}>} */ ([])
  );
...
<div class="form-control">
  <label class="label" for="badge-sel-select">
  ...
  {:else if badges.length === 0 && authorPubkey}
    <div class="label">
      <span class="label-text-alt text-warning">
        {m.badge_selector_none?.() ||
          'No badges found. Create badges in a badge management app first.'}
      </span>
    </div>
  {/if}
</div>
