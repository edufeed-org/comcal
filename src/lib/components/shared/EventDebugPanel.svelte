...
{#if debugEnabled}
	...
	<div class="border-t border-base-300 pt-4 mt-4">
		...
		{#if !showToggle || isDebugVisible}
			<div class="bg-base-200 rounded-lg p-4 space-y-4 text-xs">
				<!-- Basic Information -->
				<div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
						...
						{#if kind !== undefined}
							<div class="flex items-center justify-between bg-base-100 rounded p-2">
								...
								<code class="font-mono" title={getKindDescription(kind)}>{kind} ({getKindDescription(kind)})</code>
							</div>
						{/if}
						...
						<div class="flex items-center justify-between bg-base-100 rounded p-2">
							...
							{#if dTag}
								<code class="font-mono max-w-[200px] truncate" title={dTag}>{dTag}</code>
							{:else}
								<span class="text-error font-medium">⚠️ MISSING</span>
							{/if}
						</div>

						<!-- Created At -->
						{#if createdAt}
							<div class="flex items-center justify-between bg-base-100 rounded p-2 md:col-span-2">
				...
				<div>
					<div class="flex items-center justify-between mb-2">
						<h4 class="text-sm font-semibold text-base-content">{m.debug_panel_raw_nostr_event()}</h4>
