...
<script>
	...
	function validateRelayUrl(url) {
		if (!url.startsWith('wss://') && !url.startsWith('ws://')) {
			return m.create_community_modal_relays_validation?.() || 'Relay URL must start with wss:// or ws://';
		}
		try {
			new URL(url);
			return null;
...
<div class="card bg-base-200 p-4 mb-4">
	<h4 class="font-semibold mb-3">{contentType.name}</h4>
	...
	{#if showAdvanced}
		<div class="mt-4 pt-4 border-t border-base-300">
			<EditableList
				...
				helpText={m.content_relay_help?.() || 'Badge-gated content will use these relays instead of community relays'}
			/>
		</div>
	{/if}
</div>
