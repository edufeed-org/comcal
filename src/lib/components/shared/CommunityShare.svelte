...
<script>
			...
			for (const shareEvent of shareEvents || []) {
				if (!loadedShares.has(shareEvent.id)) {
					...
					if (aTag) {
						...
						if (pTag?.[1]) {
							shares.add(pTag[1]);
							console.log(`✅ CommunityShare: Found existing share (by event ID) for community ${pTag[1].slice(0, 8)}`);
						}
					}
				}
			}
	...
	async function deleteShare(communityPubkey) {
		console.log(`🔗 CommunityShare: Deleting share for community ${communityPubkey.slice(0, 8)}...`);
						...
						allSharesSub = eventStore
							...
							.subscribe(async (allShares) => {
								...
								if (matchingShare) {
									...
									console.warn(`🔗 CommunityShare: No share event found for community ${communityPubkey}`);
	...
	async function handleApplyShares() {
		...
		try {
			...
			console.log(`🔗 CommunityShare: Processing complete - ${successfulCount} successful, ${failedCount} failed`);
		} catch (error) {
